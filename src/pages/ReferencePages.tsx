...
import { faqs, imageAlt, locations, posts, siteSettings, testimonials } from '../data'
    ...
    <section className="section reference-about">
      <div className="container">
        ...
        <div className="reference-about-copy">
          ...
          <img src="/assets/real/obedience.webp" alt={imageAlt('/assets/real/obedience.webp','Pawrexio trainer with a dog')} loading="lazy" />
        </div>
  ...
  return <>
    ...
    <section className="section reference-blog"><div className="container">
      ...
      <div className="reference-blog-grid">{filtered.map(post => <Link to={`/blog/${post.slug}`} key={post.id}>
        <div className="reference-blog-image"><img src={post.image} alt={post.imageAlt||post.title} loading="lazy" /></div>
...
export function BlogDetail() {
  ...
  return <>
    ...
    <article className="article container"><div className="article-meta"><span>Published {post.date}</span></div><img className="article-featured-image" src={post.image} alt={post.imageAlt||title} loading="lazy"/><BlogContent content={post.content} /><LeadButton service={post.category}>Discuss your pet</LeadButton></article>
    <section className="section related"><div className="container"><SectionTitle eyebrow="Keep learning" title="Related blogs" /><div className="blog-mini-grid">{related.map(item => <Link to={`/blog/${item.slug}`} key={item.id}><img src={item.image} alt={item.imageAlt||item.title} loading="lazy" /><p>{item.category} · {item.readTime}</p><h3>{blogTitles[item.slug] || item.title}</h3><span>Read More <ArrowRight /></span></Link>)}</div></div></section>
  </>
}
...
export function Locations() {
  return <>
    ...
    <section className="section reference-locations"><div className="container reference-location-grid">{locations.map(location => <Link to={`/location/pet-training-in-${location.slug}`} key={location.id}>
      <img src={location.image} alt={location.imageAlt||`Pawrexio pet training in ${location.city}`} loading="lazy" />
