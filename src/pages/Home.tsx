...
export default function Home(){
 ...
 return <>
  ...
  <section className="section behaviour-section"><div className="container"><SectionTitle eyebrow="Pet behaviour & training" title="Is Your Pet Struggling With These Behaviours?" text="Every pet has different needs, and some behaviours can be difficult to manage on your own. Pawrexio provides professional dog and pet training services focused on practical, everyday results."/><div className="behaviour-grid">{behaviours.map(([title,text],i)=><Reveal key={title}><span>0{i+1}</span><h3>{title}</h3><p>{text}</p></Reveal>)}</div></div></section>

  <section className="consult-band"><div className="container"><div><p className="eyebrow">We're here to help</p><h2>Not Sure What Training Your Pet Needs?</h2><p>Tell us about your pet, the behaviour you're struggling with, and your location. Our professional dog trainers can help you find the right training approach and dog training service for your pet.</p></div><div className="band-actions"><LeadButton>Get personalized guidance</LeadButton><Link className="button button-ghost" to="/contact">Talk to a Pet Trainer <ArrowRight/></Link></div></div></section>
  ...
  <section className="section home-enquiry"><div className="container home-enquiry-grid"><div><h3>Send your enquiry</h3></div><HomeEnquiry/></div></section>
  ...
  <section className="section where-we-serve"><div className="container"><h2>Where We Serve</h2><div className="city-grid compact-city-grid">{locations.map((l,i)=><Link to={`/locations/${l.slug}`} key={l.id}><span>{String(i+1).padStart(2,'0')}</span><p>{l.state}</p><h3>{l.heading}</h3><p>{l.intro.slice(0,155)}…</p><b>Explore Location <ArrowRight/></b></Link>)}</div><a className="text-link" href={`tel:${siteSettings.phone}`}><Phone/>Call Pawrexio</a></div></section>
 </>
}
