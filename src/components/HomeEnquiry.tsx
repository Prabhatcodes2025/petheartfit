...
export function HomeEnquiry(){
 ...
 async function submit(e:FormEvent){
  ...
  try{
   const response=await fetch('/api/enquiries',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fullName:form.fullName,phone:form.phone,email:form.email,city:form.city,petType:'Dog',petName:'',breed:form.breed,petAge:'',service:'not-sure',concern:form.message,preferredDate:'',preferredTime:'',message:form.message,website:form.website,source:'homepage',originatingPage:'/'})})
 ...
 return <form className="booking-form home-enquiry-form" onSubmit={submit}>
  <div className="field-row"><label>Your Name *<input required minLength={2} maxLength={100} autoComplete="name" value={form.fullName} onChange={e=>setForm({...form,fullName:e.target.value})}/></label><label>Your Email<input type="email" autoComplete="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label></div>
  ...
  <label>Location *<input required maxLength={100} value={form.city} onChange={e=>setForm({...form,city:e.target.value})}/></label>
  <label>Message *<textarea required maxLength={1000} value={form.message} onChange={e=>setForm({...form,message:e.target.value})}/></label>
