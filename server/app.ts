...
const validateContent=(req:express.Request,res:express.Response,next:express.NextFunction)=>{const table=req.path.split('/')[3];const key=Object.keys(definitions).find(k=>definitions[k].table===table);if(!key)return next();const parsed=schemaFor(key).safeParse(req.body);if(!parsed.success)return res.status(400).json({message:'Invalid content fields',issues:parsed.error.flatten()});req.body=parsed.data;next()}
for(const resource of ['services','training_programs','locations','gallery','testimonials','blog_posts','faqs','seo_metadata','site_settings','enquiries']){
 ...
 app.post(`/api/admin/${resource}`,requireAdmin,validateContent,async(req,res)=>{const c=adminClient()!;const {data,error}=await c.from(resource).insert(req.body).select().single();if(error)return res.status(400).json({message:'Unable to create record'});res.status(201).json(data)})
